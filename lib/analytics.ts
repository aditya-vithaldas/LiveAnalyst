...
export const screenTool={name:'show_analytics',description:'REQUIRED for EVERY data question, before explaining it. Replace the single primary widget with the answer. display number for a single total, count, rate or value; chart for trends/graphs; table for daily details. No tabs or appended panels. mode update inherits the current dates/metrics for follow-ups; replace starts a fresh topic. Both REPLACE the primary visual. Use view customers for the ranked list, questions for examples. For ANY new metric, dimension or date range, generate plausible demo data and pass generated. Never refuse because sample data is missing. generated replaces the built-in dataset and supports arbitrary labels, values and periods. Metrics use grounded September 9–15 demo data; last 3 days is 13–15. Chart kinds: area, line, bar, pie, donut, stacked (daily category sales), horizontal (category sales), scatter (two metrics across days), funnel (sessions to carts to checkouts to orders), heatmap (daily category sales), radar (category sales), histogram (distribution of daily values for one selected metric). Pie/donut/horizontal use category sales shares. Category splits and funnel stages are illustrative demo allocations. For these category charts select sales; funnel select traffic; scatter supply two metrics. previous adds previous-week sales to area/line/bar only.',parameters:{type:'object',properties:{mode:{type:'string',enum:['replace','update']},view:{type:'string',enum:['sales','traffic','customers','questions']},display:{type:'string',enum:['number','chart','table']},query:{type:'string',description:'Pass the user question here. The page queries cloud DuckDB for demo mode, or computes uploaded rows. Do not generate values.'},generated:{type:'object',properties:{title:{type:'string'},period:{type:'string'},label:{type:'string'},unit:{type:'string',enum:['USD','%','count']},aggregation:{type:'string',enum:['sum','average']},kind:{type:'string',enum:['area','line','bar','horizontal','stacked','pie','donut','scatter','funnel','heatmap','histogram','radar']},secondaryLabel:{type:'string'},points:{type:'array',minItems:1,maxItems:120,items:{type:'object',properties:{label:{type:'string'},value:{type:'number'},secondary:{type:'number'}},required:['label','value'],additionalProperties:false}}},required:['title','period','label','unit','aggregation','kind','points'],additionalProperties:false},metrics:{type:'array',items:{type:'string',enum:['sales','traffic','orders','conversion','aov','growth']},minItems:1,maxItems:2,uniqueItems:true},kind:{type:'string',enum:['area','line','bar','pie','donut','stacked','horizontal','scatter','funnel','heatmap','histogram','radar']},start:{type:'integer',minimum:9,maximum:15},end:{type:'integer',minimum:9,maximum:15},previous:{type:'boolean'}},required:['mode','display'],additionalProperties:false}};
